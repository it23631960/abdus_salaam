...
<?php
  ...
  $professional_portfolio_audio = false;

  // Only get audio from the content if a playlist isn't present.
  if ( false === strpos( $content, 'wp-playlist-script' ) ) {
    $professional_portfolio_audio = get_media_embedded_in_content( $content, array( 'audio' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
      <?php
        if ( ! is_single() ) {
          ...
          if ( ! empty( $professional_portfolio_audio ) ) {
            foreach ( $professional_portfolio_audio as $professional_portfolio_audio_html ) {
              echo '<div class="entry-audio">';
                echo $professional_portfolio_audio_html;
